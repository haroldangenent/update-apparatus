...
module.exports = () => {
  ...
  inquirer.prompt([{
    ...
    projects.forEach(project => {
      ...
      const date = new Date();
      const branchName = `updates-${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}`;
      shelljs.exec(`git checkout -b ${branchName}`, { silent: true });
      ...
      if (fs.existsSync('composer.json')) {
        shelljs.exec(`git commit composer.json composer.lock -m "⬆️ Update dependencies\n\nAuto-update by UPDATE APPARATUS 🤖"`, { silent: true });
      }
