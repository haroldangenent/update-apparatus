...
module.exports = () => {
  ...
  inquirer.prompt([{
    ...
    if (!githubToken) {
      githubToken = readlineSync.question(chalk.yellow(`Please enter your GitHub access token [${chalk.bold('https://github.com/settings/tokens')}]: `), { hideEchoBack: true });
    ...
    projects.forEach(project => {
      ...
      shelljs.exec(`${__dirname}/../index.js --yes --skip-crawl`, { silent: true });

      const title = '⬆️ Update dependencies';
      const body = 'Auto-update by UPDATE APPARATUS 🤖';

      if (fs.existsSync('composer.json')) {
        shelljs.exec(`git add && git commit composer.json composer.lock -m "${title}\n\n${body}"`, { silent: true });
      } else {
      ...
      let repoUrl = shelljs.exec(`git config remote.origin.url`, { silent: true }).stdout.trim();
      if (repoUrl.indexOf('https://github.com/') !== -1) {
        repoUrl = repoUrl.replace('https://github.com/', 'git@github.com:');
        shelljs.exec(`git remote set-url origin ${repoUrl}`, { silent: true });
      }
